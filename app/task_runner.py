...
class ThreadPool:
    ...
    def __init__(self,logger):
        ...
        # numarul de threaduri este dat de numarul de procesoare sau de variabila de mediu TP_NUM_OF_THREADS
        ...
        for thread in self.threads:
            thread.start()
        #creez un folder in care sa pun rezultatele
        result_path = "./results"
...
class TaskRunner(Thread):
    ...
    def run(self):
        ...
        while True:
            try:
                (endpoint,recv,job_id,data) = queue.get(True,10)
            except:
    ...
    def states_mean(self,data):
        ...
        # sortez dictionarul dupa valoare
    ...
    def state_mean_by_category(self,data,state):
        ...
        for category in data:
            if state in category:
                ...
                lista_numere = [float(x) for x in lista]
                mean = sum(lista_numere) / len(lista_numere)
                #creez un nou tuplu doar din ultime 2 elemente
                #folosesc ast.literal_eval pentru a putea converti un string in tuplu
